...
public class LiquiderEngagementUseCase implements UseCase<LiquiderEngagementUseCase.Input, Void> {
    public record Input(String numeroEngagement, UUID userId) {}
    private final BudgetPlanAdapter adapter;

    @Override
    public Void execute(Input input) {
        adapter.liquiderEngagement(input.numeroEngagement(), input.userId());
        return null;
    }
}
