...
import { UUID_INEXISTANT, UUID_MALFORME, today, unique } from '../../src/data/builders';
...
test.describe('API — Écritures : saisie', () => {
  test('la saisie exige un journal', async ({ ecritureApi }) => {
    const response = await ecritureApi.saisir(
  ...
  test('la saisie exige au moins une ligne', async ({ ecritureApi }) => {
    const response = await ecritureApi.saisir(
  ...
  test('la saisie exige un numéro de pièce', async ({ ecritureApi }) => {
    const response = await ecritureApi.saisir(
  ...
  test('la saisie exige une date comptable', async ({ ecritureApi }) => {
    const response = await ecritureApi.saisir(
  ...
  test('une écriture déséquilibrée est refusée', async ({ ecritureApi, journalApi }) => {
  ...
  test('une écriture sur un journal inexistant échoue', async ({ ecritureApi }) => {
    const response = await ecritureApi.saisir(
  ...
  test('une devise hors énumération est rejetée', async ({ ecritureApi, journalApi }) => {
    const journaux = await journalApi.list();
    test.skip(journaux.length === 0, 'aucun journal en base');
  ...
  test('une écriture équilibrée est enregistrée puis relisible', async ({
    ...
    const response = await ecritureApi.saisir(ecritureEquilibree(String(journaux[0].id)), [
      200,
      201,
      ...BAD_REQUEST_STATUSES,
      404,
    ...
    const relue = await ecritureApi.getById(String(ecriture.id));
...
test.describe('API — Écritures : simulation de TVA', () => {
  test('la simulation renvoie les montants attendus', async ({ ecritureApi }) => {
    const response = await ecritureApi.simulerTva(
  ...
  test('la simulation exige un montant HT', async ({ ecritureApi }) => {
    const response = await ecritureApi.simulerTva(
  ...
  test('la simulation exige un taux', async ({ ecritureApi }) => {
    const response = await ecritureApi.simulerTva(
  ...
  test('la simulation exige un compte de charge', async ({ ecritureApi }) => {
    const response = await ecritureApi.simulerTva(
  ...
  test('un taux nul produit une TVA nulle', async ({ ecritureApi }) => {
    const response = await ecritureApi.simulerTva(
...
test.describe('API — Écritures : workflow de validation', () => {
  test('soumettre une écriture inexistante échoue', async ({ ecritureApi }) => {
    const response = await ecritureApi.soumettre(UUID_INEXISTANT, NOT_FOUND_STATUSES);

    await expectStatusIn(response, NOT_FOUND_STATUSES, 'soumission écriture inexistante');
  });

  test('valider une écriture inexistante échoue', async ({ ecritureApi }) => {
    const response = await ecritureApi.valider(UUID_INEXISTANT, NOT_FOUND_STATUSES);

    await expectStatusIn(response, NOT_FOUND_STATUSES, 'validation écriture inexistante');
  });

  test('rejeter une écriture inexistante échoue', async ({ ecritureApi }) => {
    const response = await ecritureApi.rejeter(UUID_INEXISTANT, NOT_FOUND_STATUSES);

    await expectStatusIn(response, NOT_FOUND_STATUSES, 'rejet écriture inexistante');
  });

  test('consulter une écriture inexistante ne renvoie pas 200', async ({ ecritureApi }) => {
    const response = await ecritureApi.getById(UUID_INEXISTANT, NOT_FOUND_STATUSES);

    await expectStatusIn(response, NOT_FOUND_STATUSES, 'écriture inexistante');
  });

  test('un identifiant malformé est rejeté', async ({ ecritureApi }) => {
    const response = await ecritureApi.getById(UUID_MALFORME, BAD_REQUEST_STATUSES);
  ...
  test('le cycle saisie → soumission → validation est cohérent', async ({
    ...
    const creation = await ecritureApi.saisir(ecritureEquilibree(String(journaux[0].id)), [
      200,
      201,
      ...BAD_REQUEST_STATUSES,
      404,
