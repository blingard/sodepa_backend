...
import { UUID_INEXISTANT, UUID_MALFORME, decisionValide, unique } from '../../src/data/builders';

test.describe('API — Utilisateurs : recherche et consultation', () => {
  test('la liste paginée est cohérente', async ({ userApi }) => {
    const page = await userApi.page({ page: 0, size: 10 });

    expectValidPage(page, { expectedPage: 0, expectedSize: 10 });
  });

  test('la seconde page ne répète pas la première', async ({ userApi }) => {
    const premiere = await userApi.page({ page: 0, size: 2 });
    test.skip(premiere.totalElements < 3, 'moins de trois utilisateurs en base');

    const seconde = await userApi.page({ page: 1, size: 2 });

    const ids = premiere.content.map((u) => u.id);
    expect(seconde.content.filter((u) => ids.includes(u.id))).toHaveLength(0);
  });

  test('la file maker-checker est paginée', async ({ userApi }) => {
    const page = await userApi.pending({ page: 0, size: 10 });

    expectValidPage(page, { expectedPage: 0, expectedSize: 10 });
  });

  test('la recherche sans critère renvoie tous les utilisateurs', async ({ userApi }) => {
    const tous = await userApi.page({ page: 0, size: 1 });
    const recherche = await userApi.search({ page: 0, size: 1 });

    expect(recherche.totalElements).toBe(tous.totalElements);
  });

  test('la recherche par nom filtre le résultat', async ({ userApi }) => {
    const reference = await userApi.page({ page: 0, size: 1 });
    ...
    const resultats = await userApi.search({ page: 0, size: 20, nom });
  ...
  test('la recherche par email filtre le résultat', async ({ userApi }) => {
    const reference = await userApi.page({ page: 0, size: 1 });
    test.skip(reference.content.length === 0, 'aucun utilisateur en base');
    ...
    const resultats = await userApi.search({ page: 0, size: 20, email });

    expect(resultats.totalElements).toBeGreaterThan(0);
  });

  test('une recherche sans correspondance renvoie une page vide', async ({ userApi }) => {
    const resultats = await userApi.search({ page: 0, size: 10, nom: unique('ZZZ') });

    expect(resultats.content).toHaveLength(0);
    expect(resultats.empty).toBe(true);
  });

  test('les critères de recherche se combinent', async ({ userApi }) => {
    const resultats = await userApi.search({
      page: 0,
      size: 10,
      nom: unique('ZZZ'),
      prenom: unique('YYY'),
  ...
  test('la consultation unitaire renvoie l’utilisateur demandé', async ({ userApi }) => {
    const page = await userApi.page({ page: 0, size: 1 });
    test.skip(page.content.length === 0, 'aucun utilisateur en base');

    const response = await userApi.getById(String(page.content[0].id));
    const utilisateur = (await response.json()) as Record<string, unknown>;

    expect(String(utilisateur.id)).toBe(String(page.content[0].id));
  });

  test('un identifiant inexistant ne renvoie pas 200', async ({ userApi }) => {
    const response = await userApi.getById(UUID_INEXISTANT, NOT_FOUND_STATUSES);

    await expectStatusIn(response, NOT_FOUND_STATUSES, 'utilisateur inexistant');
  });

  test('un identifiant malformé est rejeté', async ({ userApi }) => {
    const response = await userApi.getById(UUID_MALFORME, BAD_REQUEST_STATUSES);
...
test.describe('API — Utilisateurs : mise à jour', () => {
  test('la mise à jour exige un email bien formé', async ({ userApi }) => {
    const response = await userApi.initUpdate(
      UUID_INEXISTANT,
  ...
  test('la mise à jour exige au moins un téléphone', async ({ userApi }) => {
    const response = await userApi.initUpdate(
      UUID_INEXISTANT,
  ...
  test('la mise à jour exige un nom', async ({ userApi }) => {
    const response = await userApi.initUpdate(
      UUID_INEXISTANT,
  ...
  test('la mise à jour d’un utilisateur inexistant échoue', async ({ userApi }) => {
    const response = await userApi.initUpdate(
      UUID_INEXISTANT,
      {
        nom: 'Test',
  ...
  test('la mise à jour des permissions exige une liste non vide', async ({ userApi }) => {
    const response = await userApi.initUpdatePermissions(UUID_INEXISTANT, { permissions: [] }, [
  ...
  test('une permission hors énumération est rejetée', async ({ userApi }) => {
    const response = await userApi.initUpdatePermissions(
      UUID_INEXISTANT,
  ...
  test('valider une soumission inexistante échoue', async ({ userApi }) => {
    const response = await userApi.validateOrReject(
      UUID_INEXISTANT,
      decisionValide(),
      NOT_FOUND_STATUSES,
    );
  ...
  test('une décision sans notes est refusée', async ({ userApi }) => {
    const response = await userApi.validateOrReject(
      UUID_INEXISTANT,
